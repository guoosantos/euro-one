...
import { buildVehicleTable, events } from "../mock/fleet";
...
export function Topbar({ title }) {
  ...
  const searchResults = useMemo(() => {
    ...
    return buildVehicleTable(tenantId)
      .filter((item) => item.name.toLowerCase().includes(term) || item.plate.toLowerCase().includes(term))
      .slice(0, 5);
  }, [query, tenantId]);

  const recentEvents = useMemo(() => events.filter((item) => item.tenantId === tenantId).slice(0, 3), [tenantId]);
  ...
  return (
    <header className="sticky top-0 z-20 bg-[#0b0f17]/95 backdrop-blur border-b border-white/5">
            ...
            <div className="absolute left-0 right-0 top-full z-10 mt-2 overflow-hidden rounded-xl border border-white/10 bg-[#0f141c] shadow-2xl">
              ...
              <ul>
                {recentEvents.map((event) => (
                  <li key={event.id} className="px-4 py-2 text-sm text-white/70">
