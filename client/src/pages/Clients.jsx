...
export default function Clients() {
  ...
  const currentClient = useMemo(() => {
    ...
    return clients.find((entry) => entry.id === tenant.id) ?? {
      id: tenant.id,
      name: tenant.name,
      email: tenant.email,
    ...
    return (
      <div className="space-y-4 text-white/80">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
          <div className="text-lg font-semibold text-white">{currentClient?.name ?? "Minha empresa"}</div>
          <dl className="mt-3 space-y-2 text-sm text-white/70">
            ...
            <div className="flex justify-between">
              <dt className="opacity-60">Limite de dispositivos</dt>
              <dd>{currentClient?.deviceLimit ?? "—"}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="opacity-60">Limite de usuários</dt>
              <dd>{currentClient?.userLimit ?? "—"}</dd>
            </div>
          </dl>
          <p className="mt-4 text-xs text-amber-200/80">
            Criação e edição de clientes é exclusiva para administradores Euro One.
          </p>
        </div>
      </div>
    );
      ...
      <section className="card space-y-4">
        <header className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs opacity-70">Crie locatários e defina limites de dispositivos e usuários.</p>
          </div>
          <button
            type="button"
            onClick={() => {
