...
export default function useMonitoringSettings({
  ...
  const mergePrefs = useCallback((saved) => mergeColumnPreferences(defaults, saved), [defaults]);
  ...
  useEffect(() => {
    ...
    const merged = mergePrefs(remotePreferences?.monitoringTableColumns || localPrefs);
    setColumnPrefs(merged);

    const remoteRatio = Number(remotePreferences?.monitoringPanelRatio);
    if (Number.isFinite(remoteRatio) && remoteRatio > 0 && remoteRatio < 1) {
      setPanelRatio(remoteRatio);
    } else {
      ...
      if (Number.isFinite(storedRatio) && storedRatio > 0 && storedRatio < 1) {
        setPanelRatio(storedRatio);
      }
    }
  }, [columns?.length, defaults, mergePrefs, remotePreferences, storageKey]);

  const persistColumns = useCallback(
    (nextPrefs) => {
      saveColumnPreferences(storageKey, nextPrefs);
      if (typeof savePreferences === "function" && !loadingPreferences) {
        savePreferences({ monitoringTableColumns: { visible: nextPrefs.visible, order: nextPrefs.order } }).catch((error) => {
  ...
  const visibleColumns = useMemo(() => resolveVisibleColumns(columns, columnPrefs), [columnPrefs, columns]);
